...
def statistical_analysis(data: np.ndarray) -> dict:
    ...
    return {"средний балл": np.mean(data), 
            ...
            "25 перцентиль": np.percentile(data, 25), 
            "75 перцентиль": np.percentile(data, 75)}
...
def plot_histogram(data: np.ndarray):
    ...
    """
    plt.hist(data)
    plt.title("Гисторнамма распределения оценок по математике")
    plt.xlabel("Оценка")
    plt.ylabel("Студентов")
    plt.savefig("codeLabs/numpy_lab/plots/histogram.png")
    plt.close()

data = load_dataset("codeLabs/numpy_lab/data/students_scores.csv")
...
def plot_heatmap(matrix: np.ndarray):
    ...
    plt.savefig("codeLabs/numpy_lab/plots/heatmap.png")
    plt.close()
...
def plot_line(x: np.ndarray, y: np.ndarray):
    ...
    plt.savefig("codeLabs/numpy_lab/plots/line.png")
    plt.close()
